...
Notebook은 실험 및 개발에 이상적입니다. 기계 학습 모델을 개발했고 프로덕션 준비가 되면 스크립트를 사용하여 학습할 수 있습니다. 스크립트를 명령 작업으로 실행할 수 있습니다.
...
컴퓨팅 instance 연결된 Notebook을 사용하면 작성한 코드를 즉시 실행하고 출력을 검토할 수 있으므로 실험 및 개발에 이상적입니다. 개발에서 프로덕션으로 이동하려면 스크립트를 사용해야 합니다. 첫 번째 단계로 Azure Machine Learning 스튜디오 사용하여 Notebook을 스크립트로 변환할 수 있습니다.
...
Notebook을 스크립트로 변환한 후 더 구체화할 수 있습니다. 스크립트를 사용할 때 한 가지 모범 사례는 함수를 사용하는 것입니다. 스크립트가 함수로 구성되면 코드를 단위 테스트하는 것이 더 쉽습니다. 함수를 사용하는 경우 스크립트는 특정 작업을 수행하는 각 블록의 코드 블록으로 구성됩니다.

1. **Labs/08/src/train-model-parameters.py** 스크립트를 열고 해당 내용을 탐색합니다.
    4개의 다른 함수를 포함하는 기본 함수가 있습니다.
    ...
    기본 함수 다음에는 각 함수가 정의됩니다. 각 함수가 예상되는 입력 및 출력을 정의하는 방법을 확인합니다.

1. Notebook 맨 위에 있는 &#9655;&#9655; 아이콘을 선택하여 **터미널에서 스크립트를 저장하고 실행**합니다. **데이터 읽기** 후 파일 경로가 잘못되어 데이터를 가져올 수 없다는 오류가 발생합니다.
    ...
    다음과 같은 두 가지 입력 매개 변수가 정의되어 있습니다.
    - **--training_data** 문자열을 예상합니다.
    - **--reg_rate** 숫자를 예상하지만 기본값은 0.01입니다.

    스크립트를 성공적으로 실행하려면 학습 데이터 매개 변수의 값을 지정해야 합니다. 학습 스크립트와 동일한 폴더에 저장된 **diabetes.csv** 파일을 참조하여 이 작업을 수행해 보겠습니다.
...
스크립트가 성공적으로 실행되고 결과적으로 출력에 학습된 모델의 정확도 및 AUC가 표시됩니다.

터미널에서 스크립트를 테스트하는 것은 스크립트가 예상대로 작동하는지 확인하는 데 적합합니다. 코드에 문제가 있는 경우 터미널에서 오류가 발생합니다.

**필요에 따라** 코드를 편집하여 오류를 적용하고 터미널에서 명령을 다시 실행하여 스크립트를 실행합니다. 예를 들어 줄 **가져오기 pandas를 pd로** 제거하고 입력 매개 변수를 사용하여 스크립트를 저장하고 실행하여 오류 메시지를 검토합니다.
...
**필요에 따라** 코드를 편집하여 오류를 강제로 적용하고 Notebook을 사용하여 명령 작업을 다시 시작합니다. 예를 들어 스크립트에서 줄 **가져오기 pandas를 pd로** 제거하고 스크립트를 저장합니다. 또는 명령 작업 구성을 편집하여 스크립트 대신 작업 구성 자체에 문제가 있는 경우 오류 메시지를 탐색합니다.
