...
Notebook은 실험 및 개발에 적합합니다. 기계 학습 모델을 개발하고 프로덕션에 사용할 준비가 되면 스크립트를 사용하여 학습하려고 합니다. 스크립트를 명령 작업으로 실행할 수 있습니다.
...
컴퓨팅 인스턴스에 연결된 Notebook을 사용하면 작성한 코드를 즉시 실행하고 출력을 검토할 수 있으므로 실험 및 개발에 적합합니다. 개발에서 프로덕션으로 이동하려면 스크립트를 사용해야 합니다. 첫 번째 단계로 Azure Machine Learning 스튜디오 사용하여 Notebook을 스크립트로 변환할 수 있습니다.
...
1. &#9776을 선택합니다. 전자 필기장 메뉴를** 보려면 전자 필기장 맨 위에 있는 아이콘입니다**.
1. 다른 사용자로** 내보내기를 확장하고 **Python(.py)**을 선택하여 **Notebook을 Python 스크립트로 변환합니다.
1. 새 파일 `train-classification-model`의 이름을 지정합니다.
1. 새 파일이 만들어지면 스크립트가 자동으로 열립니다. 파일을 탐색하고 Notebook과 동일한 코드가 포함되어 있음을 확인합니다.
1. 선택 된 &#9655; &#9655; 전자 필기장 맨 위에 있는 아이콘을 사용하여 **터미널**에서 스크립트를 저장하고 실행합니다.
1. 스크립트는 명령 **python train-classification-model.py** 의해 시작되며 출력은 명령 아래에 표시되어야 합니다.

## 터미널을 사용하여 스크립트 테스트

Notebook을 스크립트로 변환한 후에는 더 구체화할 수 있습니다. 스크립트를 사용할 때 가장 좋은 방법은 함수를 사용하는 것입니다. 스크립트가 함수로 구성된 경우 코드를 단위 테스트하는 것이 더 쉽습니다. 함수를 사용하는 경우 스크립트는 특정 작업을 수행하는 각 블록의 코드 블록으로 구성됩니다.

1. **Labs/08/src/train-model-parameters.py** 스크립트를 열고 해당 내용을 탐색합니다.
    다른 4개의 함수를 포함하는 기본 함수가 있습니다.
    ...
    기본 함수 후에 각 함수가 정의됩니다. 각 함수가 예상된 입력 및 출력을 정의하는 방법을 확인합니다.

1. 선택 된 &#9655; &#9655; 전자 필기장 맨 위에 있는 아이콘을 사용하여 **터미널**에서 스크립트를 저장하고 실행합니다. 데이터를** 읽은 후 **파일 경로가 잘못되어 데이터를 가져올 수 없다는 오류가 발생합니다.
    ...
    - **--training_data** 문자열이 필요합니다.
    - **--reg_rate** 숫자를 예상하지만 기본값은 0.01입니다.

    스크립트를 성공적으로 실행하려면 학습 데이터 매개 변수의 값을 지정해야 합니다. 학습 스크립트와 동일한 폴더에 저장된 diabetes.csv** 파일을 참조**하여 수행해 보겠습니다.
...
**필요에 따라** 코드를 편집하여 오류를 강제로 적용하고 터미널에서 명령을 다시 실행하여 스크립트를 실행합니다. 예를 들어 pd**로 pandas 줄 **가져오기를 제거하고 입력 매개 변수를 사용하여 스크립트를 저장하고 실행하여 오류 메시지를 검토합니다.

## 명령 작업으로 스크립트 실행

스크립트가 작동하는지 알고 있는 경우 명령 작업으로 실행할 수 있습니다. 스크립트를 명령 작업으로 실행하면 스크립트의 모든 입력 및 출력을 추적할 수 있습니다.

1. Labs/08/Run 스크립트를 **command job.ipynb** Notebook으로 엽니다.
1. Notebook의 모든 셀을 실행합니다.
1. Azure Machine Learning 스튜디오 작업** 페이지로 **이동합니다.
1. **당뇨병 학습 스크립트** 작업으로 이동하여 실행한 명령 작업의 개요를 살펴봅니다.
1. 코드** 탭으로 **이동합니다. 명령 작업의 코드** 매개 변수 값**이었던 src** 폴더의 모든 내용**이 여기에 복사됩니다. 명령 작업에서 실행한 학습 스크립트를 검토할 수 있습니다.
1. **출력 + 로그 탭으로** 이동합니다.
...
**필요에 따라** 코드를 편집하여 오류를 강제로 적용하고 Notebook을 사용하여 명령 작업을 다시 시작합니다. 예를 들어 스크립트에서 줄 **가져오기 pandas를 pd** 로 제거하고 스크립트를 저장합니다. 또는 명령 작업 구성을 편집하여 스크립트 대신 작업 구성 자체에 문제가 있는 경우 오류 메시지를 탐색합니다.
