...
애플리케이션에서 모델을 사용하고 실시간 예측을 가져오려면 모델을 관리형 온라인 엔드포인트에 배포해야 합니다. MLflow 모델은 환경을 정의하거나 채점 스크립트를 만들 필요가 없으므로 쉽게 배포됩니다.

이 연습에서는 MLflow 모델을 관리형 온라인 엔드포인트에 배포하고 샘플 데이터에서 테스트합니다. 
