...
## 자동화된 Machine Learning으로 분류 모델 학습

이제 필요한 리소스가 모두 준비되었으므로 Notebook을 실행하여 자동화된 Machine Learning 작업을 구성하고 제출할 수 있습니다.
...
1. Notebook의 모든 셀을 실행합니다.
    ...
    Azure Machine Learning 작업 영역에 새 작업이 만들어집니다. 작업은 작업 구성에 정의된 입력, 사용된 데이터 자산, 모델을 평가하기 위한 메트릭과 같은 출력을 추적합니다.

    자동화된 Machine Learning 작업에는 학습된 개별 모델과 실행에 필요한 기타 작업을 나타내는 자식 작업이 포함되어 있습니다.
1. **작업**으로 이동하여 **auto-ml-class-dev** 실험을 선택합니다.
1. **표시 이름** 열에서 작업을 선택합니다.
1. 상태가 **완료됨**으로 변경될 때까지 기다립니다.
1. Machine Learning 자동화 작업 상태가 **완료됨**으로 변경되면 스튜디오에서 작업 세부 정보를 살펴봅니다.
    - **데이터 가드 레일** 탭에는 학습 데이터에 문제가 있는지 여부가 표시됩니다.
    - **모델 + 자식 작업** 탭에는 학습된 모든 모델이 표시됩니다. 가장 적절한 모델에 대해 **모델 설명**을 선택하고 **aml-cluster**를 사용하여 설명 작업 실행을 만듭니다.
    - **알고리즘 이름** 옆에 새로 **설명됨** 열이 나타날 때까지 기다려 **설명 보기**를 선택합니다. 알고리즘 목록을 새로 고쳐야 이 옵션이 표시될 수도 있습니다.
    - 만든 대시보드를 검토하여 대상 값에 가장 큰 영향을 준 기능을 파악합니다.
