---
lab:
  title: 자동화된 Machine Learning을 사용하여 최상의 분류 모델 찾기
---

# 자동화된 Machine Learning을 사용하여 최상의 분류 모델 찾기

모델 학습에 적합한 알고리즘 및 전처리 변환을 결정하려면 많은 추측과 실험이 필요할 수 있습니다.

이 연습에서는 자동화된 기계 학습을 사용하여 여러 학습 실행을 병렬로 수행하여 모델의 최적 알고리즘 및 전처리 단계를 결정합니다.
...
Azure CLI를 사용하여 작업 영역 및 필요한 컴퓨팅을 프로비전하고 Python SDK를 사용하여 자동화된 Machine Learning을 사용하여 분류 모델을 학습시킵니다.
...
## 자동화된 기계 학습을 사용하여 분류 모델 학습

이제 필요한 모든 리소스가 있으므로 Notebook을 실행하여 자동화된 Machine Learning 작업을 구성하고 제출할 수 있습니다.

1. **자동화된 Machine Learning.ipynb** Notebook을 사용하여 Labs/06/Classification을 엽니다.
...
1. Notebook의 모든 셀을 실행합니다.
    ...
    Azure Machine Learning 작업 영역에 새 작업이 만들어집니다. 작업은 작업 구성에 정의된 입력, 사용된 데이터 자산 및 모델을 평가하는 메트릭과 같은 출력을 추적합니다.

    자동화된 Machine Learning 작업에는 학습된 개별 모델과 실행하는 데 필요한 기타 작업을 나타내는 자식 작업이 포함됩니다.
1. 작업으로 **이동하여 auto-ml-class-dev** 실험을 선택합니다**.**
1. 표시 이름** 열 아래에서 **작업을 선택합니다.
1. 상태 완료**됨으로 변경될 **때까지 기다립니다.
1. Machine Learning 자동화 작업 상태 완료**됨으로 **변경되면 스튜디오에서 작업 세부 정보를 탐색합니다.
    - 데이터 가드레일** 탭에는 **학습 데이터에 문제가 있는지 여부가 표시됩니다.
    - 모델** 탭에는 **학습된 모든 모델이 표시됩니다. 가장 적합한 모델에 대한 보기 설명을** 선택하여 **대상 값에 가장 큰 영향을 준 기능을 파악합니다.
