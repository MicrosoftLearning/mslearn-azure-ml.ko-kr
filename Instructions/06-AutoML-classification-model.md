...
1. **자동화된 Machine Learning.ipynb Notebook을 사용하여 Labs/06/Classification**을 엽니다.
...
Azure Machine Learning 작업 영역에 새 작업이 만들어집니다. 작업은 작업 구성에 정의된 입력, 사용된 데이터 자산 및 모델을 평가하기 위한 메트릭과 같은 출력을 추적합니다.

자동화된 Machine Learning 작업에는 학습된 개별 모델과 실행하는 데 필요한 기타 작업을 나타내는 자식 작업이 포함됩니다. 

3. Machine Learning 자동화 작업이 완료되면 스튜디오에서 작업 세부 정보를 탐색합니다.
    - **데이터 가드레일 탭에는** 학습 데이터에 문제가 있는지 여부가 표시됩니다.
    - **모델** 탭에는 학습된 모든 모델이 표시됩니다.
    - 가장 적합한 모델에 대한 **설명 보기를** 선택하여 대상 값에 가장 큰 영향을 준 기능을 파악합니다.
