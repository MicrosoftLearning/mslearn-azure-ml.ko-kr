---
lab:
  title: MLflow를 사용하여 Notebook에서 모델 학습 추적
---

# MLflow를 사용하여 Notebook에서 모델 학습 추적

여러 모델을 실험하고 학습하여 새 데이터 과학 프로젝트를 시작하는 경우가 많습니다. 작업을 추적하고 학습하는 모델 및 수행 방법에 대한 개요를 유지하려면 MLflow 추적을 사용할 수 있습니다.

이 연습에서는 모델 학습을 기록하는 컴퓨팅 instance 실행되는 Notebook 내에서 MLflow를 수행합니다.
...
1. Azure Machine Learning 스튜디오 내에서 **컴퓨팅** 페이지로 이동하여 이전 섹션에서 만든 컴퓨팅 instance 있는지 확인합니다. 컴퓨팅 instance 실행 중이어야 합니다.
1. **컴퓨팅 인스턴스** 탭에서 컴퓨팅 instance 찾아 **터미널** 애플리케이션을 선택합니다.
1. 터미널에서 터미널에서 다음 명령을 실행하여 컴퓨팅 instance Python SDK 및 MLflow 라이브러리를 설치합니다.
...
이제 필요한 모든 리소스가 있으므로 Notebook에서 모델을 학습할 때 Notebook을 실행하여 MLflow를 구성하고 사용할 수 있습니다.

1. **MLflow.ipynb Notebook을 사용하여 Labs/07/Track 모델 학습**을 엽니다.

    > **인증을** 선택하고 인증을 요청하는 알림이 표시되면 필요한 단계를 따릅니다.

1. Notebook에서 **Python 3.8 - AzureML** 커널을 사용하는지 확인합니다.
1. Notebook의 모든 셀을 실행합니다.
1. 모델을 학습할 때마다 생성된 새 작업을 검토합니다.
