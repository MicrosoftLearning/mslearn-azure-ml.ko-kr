...
대다수 시나리오에서 유추는 예측 모델을 사용해 여러 사례의 점수를 매기는 일괄 처리 프로세스로 수행됩니다. Azure Machine Learning에서 이러한 종류의 추론 솔루션을 구현하려면 일괄 처리 엔드포인트를 만들 수 있습니다.

이 연습에서는 일괄 처리 엔드포인트에 MLflow 모델을 배포하고 작업을 제출하여 샘플 데이터에서 테스트합니다. 
...
1. **Labs/11/Deploy to batch endpoint.ipynb Notebook을** 엽니다.
