---
lab:
  title: 일괄 처리 엔드포인트에 모델 배포
---

# 일괄 처리 엔드포인트에 모델 배포

많은 시나리오에서 추론은 예측 모델을 사용하여 많은 수의 사례를 채점하는 일괄 처리 프로세스로 수행됩니다. Azure Machine Learning에서 이러한 종류의 추론 솔루션을 구현하려면 일괄 처리 엔드포인트를 만들 수 있습니다.

이 연습에서는 MLflow 모델을 일괄 처리 엔드포인트에 배포하고 작업을 제출하여 샘플 데이터에서 테스트합니다.
...
엔드포인트를 만들고 Python SDK를 사용하여 MLflow 모델을 배포하는 코드가 Notebook에 제공됩니다.

1. **Labs/11/Deploy를 열어 batch endpoint.ipynb** Notebook에 배포합니다.
